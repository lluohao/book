...
public class UpdateBookServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {
					...
					if (fileExtName.matches("^(jpg|gif|png)$")) {
						savePath = imgPath;
					} else if (fileExtName.matches("^(pdf|txt|docx)$")) {
						savePath = bookPath;
					}
		...
		Book b=service.findBookByName(bookName);
...
//		System.out.println("2222"+bookPath);
		book.setDiscount(discountInt);
//		System.out.println("lllll"+discountInt);
		book.setDiscribe(describe);
//		System.out.println(")))))))))))))"+describe);
		book.setImg(imgPath+"\\"+filename);
		book.setName(bookName);
//		System.out.println("3333"+bookName);
		...
		boolean boo = service.updateBook(book);
		...
		request.getRequestDispatcher("/ManagerUpdateBook.jsp").forward(request, response);
